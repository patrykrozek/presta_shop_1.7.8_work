...
class OmnibusEuFree extends Module
{
    ...
    $sql->where('is_last IN (0, 1)');
        ...
        $sqlResult = Db::getInstance()->executeS($sql);
            ...
            // Logowanie przed zwróceniem wyniku
    if (!empty($sqlResult)) {
        PrestaShopLogger::addLog('Wynik SQL: ' . print_r($sqlResult[0], true), 1);
    } else {
        PrestaShopLogger::addLog('Wynik SQL: Brak danych', 1);
    }
...
// Stary kod - orginał
    public function hookDisplayOmnibusEuFree($params)
    {
        ...
        if (!empty($lastMinimalPrice)) {
         ...
         // Rozek logowania blok: 
        
        // Logowanie ID produktu
    PrestaShopLogger::addLog('Rozek hookDisplayOmnibusEuFree wywołany dla produktu ID: ' . (isset($params['product']['id_product']) ? $params['product']['id_product'] : 'brak produktu'), 1);

    // Pobranie waluty
    $currency = $this->context->currency;

    // Pobranie minimalnej ceny z ostatnich X dni
    $lastMinimalPrice = $this->getLastMinimalPrice($params['product']['id_product'], $params['product']['id_product_attribute'], $currency->id);

    // Logowanie wyników getLastMinimalPrice
    PrestaShopLogger::addLog('Rozek Wynik getLastMinimalPrice: ' . (!empty($lastMinimalPrice) ? print_r($lastMinimalPrice, true) : 'Brak danych'), 1);

    // Formatowanie minimalnej ceny
    if (!empty($lastMinimalPrice)) {
        $minimalPrice = PrestashopCompatibility::formatPrice(_PS_VERSION_, $lastMinimalPrice['price'], $currency);

        // Logowanie sformatowanej minimalnej ceny
        PrestaShopLogger::addLog('Rozek Sformatowana minimalna cena: ' . $minimalPrice, 1);
    } else {
        $minimalPrice = null;

        // Logowanie braku minimalnej ceny
        PrestaShopLogger::addLog(' Rozek Minimalna cena nie została znaleziona.', 1);
    }

    // Logowanie bieżącej ceny produktu
    PrestaShopLogger::addLog('Rozek Bieżąca cena produktu: ' . $params['product']['price'], 1);
       
        PrestaShopLogger::addLog('Obecna Minimalna cena Rozka2: ' . (isset($minimalPrice) ? $minimalPrice : 'Brak danych'), 1);

 
        //Koniec Logowan Rozka
        ...
        $this->context->smarty->assign([
            ...
            //'OmnibuseufreeProductDiscount' => true,  // ustawiamy na sztywno na true
...
{
    // Logowanie ID produktu
    PrestaShopLogger::addLog('hookDisplayOmnibusEuFree wywołany dla produktu ID: ' . (isset($params['product']['id_product']) ? $params['product']['id_product'] : 'brak produktu'), 1);
    ...
    // Logowanie wyników getLastMinimalPrice
    PrestaShopLogger::addLog('Wynik getLastMinimalPrice: ' . (!empty($lastMinimalPrice) ? print_r($lastMinimalPrice, true) : 'Brak danych'), 1);
    ...
    if (!empty($lastMinimalPrice)) {
        ...
        // Logowanie sformatowanej minimalnej ceny
        PrestaShopLogger::addLog('Sformatowana minimalna cena: ' . $minimalPrice, 1);
    } else {
        $minimalPrice = null;

        // Logowanie braku minimalnej ceny
        PrestaShopLogger::addLog('Minimalna cena nie została znaleziona.', 1);
    }

    // Logowanie bieżącej ceny produktu
    PrestaShopLogger::addLog('Bieżąca cena produktu: ' . $params['product']['price'], 1);

    return ''; // Funkcja nic nie zwraca dla tego przypadku (brak interakcji z Smarty)
}
*/
